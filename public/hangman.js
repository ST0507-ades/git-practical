...
// HINT
const gamerealResult = document.getElementById('hangman-result');
...
function startGame(word = words[Math.floor(Math.random() * words.length)], maxGuesses = 6) {
    ...
    const processGuess = (guess) => {
        if (guessedLetters.has(guess)) {
            gamerealResult.textContent = 'You already guessed that letter.';
        } else if (guess.length !== 1) {
            gamerealResult.textContent = 'Please enter a single letter.';
        } else if (!/[a-z]/.test(guess)) {
            gamerealResult.textContent = 'Please enter a letter.';
        } else {
            ...
            if (word.includes(guess)) {
                ...
                if (!wordDisplay.textContent.includes('_')) {
                    gamerealResult.textContent = 'Congratulations, you won!';
                    disableButtons();
                }
            } else {
                ...
                if (guessesRemaining === 0) {
                    gamerealResult.textContent = `Sorry, you lost. The word was "${word}".`;
                    disableButtons();
                } else {
                    gamerealResult.textContent = 'Incorrect guess.';
                }
            }
