...
// HINT
const game2Result = document.getElementById('hangman-result');
...
function startGame(word = words[Math.floor(Math.random() * words.length)], maxGuesses = 6) {
    ...
    const processGuess = (guess) => {
        if (guessedLetters.has(guess)) {
            ...
            game2Result.textContent = 'Please enter a letter.';
        } else {
            ...
            if (word.includes(guess)) {
                ...
                if (!wordDisplay.textContent.includes('_')) {
                    game2Result.textContent = 'Congratulations, you won!';
                    disableButtons();
                }
            } else {
                ...
                if (guessesRemaining === 0) {
                    game2Result.textContent = `Sorry, you lost. The word was "${word}".`;
                    disableButtons();
                } else {
                    game2Result.textContent = 'Incorrect guess.';
                }
            }
