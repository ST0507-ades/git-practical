...
// HINT
const HangResult = document.getElementById('hangman-result');
...
function startGame(word = words[Math.floor(Math.random() * words.length)], maxGuesses = 6) {
    ...
    const processGuess = (guess) => {
        if (guessedLetters.has(guess)) {
            HangResult.textContent = 'You already guessed that letter.';
        } else if (guess.length !== 1) {
            HangResult.textContent = 'Please enter a single letter.';
        } else if (!/[a-z]/.test(guess)) {
            HangResult.textContent = 'Please enter a letter.';
        } else {
            ...
            if (word.includes(guess)) {
                ...
                if (!wordDisplay.textContent.includes('_')) {
                    HangResult.textContent = 'Congratulations, you won!';
                    disableButtons();
                }
            } else {
                ...
                if (guessesRemaining === 0) {
                    HangResult.textContent = `Sorry, you lost. The word was "${word}".`;
                    disableButtons();
                } else {
                    HangResult.textContent = 'Incorrect guess.';
                }
            }
