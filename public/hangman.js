...
// HINT
const gameRes = document.getElementById('hangman-result');
...
function startGame(word = words[Math.floor(Math.random() * words.length)], maxGuesses = 6) {
    ...
    const processGuess = (guess) => {
        if (guessedLetters.has(guess)) {
            gameRes.textContent = 'You already guessed that letter.';
        } else if (guess.length !== 1) {
            gameRes.textContent = 'Please enter a single letter.';
        } else if (!/[a-z]/.test(guess)) {
            gameRes.textContent = 'Please enter a letter.';
        } else {
            ...
            if (word.includes(guess)) {
                ...
                if (!wordDisplay.textContent.includes('_')) {
                    gameRes.textContent = 'Congratulations, you won!';
                    disableButtons();
                }
            } else {
                ...
                if (guessesRemaining === 0) {
                    gameRes.textContent = `Sorry, you lost. The word was "${word}".`;
                    disableButtons();
                } else {
                    gameRes.textContent = 'Incorrect guess.';
                }
            }
