...
// HINT
const gameResult_hangman = document.getElementById('hangman-result');
...
function startGame(word = words[Math.floor(Math.random() * words.length)], maxGuesses = 6) {
    ...
    const processGuess = (guess) => {
        if (guessedLetters.has(guess)) {
            gameResult_hangman.textContent = 'You already guessed that letter.';
        } else if (guess.length !== 1) {
            gameResult_hangman.textContent = 'Please enter a single letter.';
        } else if (!/[a-z]/.test(guess)) {
            gameResult_hangman.textContent = 'Please enter a letter.';
        } else {
            ...
            if (word.includes(guess)) {
                ...
                if (!wordDisplay.textContent.includes('_')) {
                    gameResult_hangman.textContent = 'Congratulations, you won!';
                    disableButtons();
                }
            } else {
                ...
                if (guessesRemaining === 0) {
                    gameResult_hangman.textContent = `Sorry, you lost. The word was "${word}".`;
                    disableButtons();
                } else {
                    gameResult_hangman.textContent = 'Incorrect guess.';
                }
            }
