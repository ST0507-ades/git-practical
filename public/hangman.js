...
// HINT
const gameResultss = document.getElementById('hangman-result');
...
function startGame(word = words[Math.floor(Math.random() * words.length)], maxGuesses = 6) {
    ...
    const processGuess = (guess) => {
        if (guessedLetters.has(guess)) {
            gameResultss.textContent = 'You already guessed that letter.';
        } else if (guess.length !== 1) {
            gameResultss.textContent = 'Please enter a single letter.';
        } else if (!/[a-z]/.test(guess)) {
            gameResultss.textContent = 'Please enter a letter.';
        } else {
            ...
            if (word.includes(guess)) {
                ...
                if (!wordDisplay.textContent.includes('_')) {
                    gameResultss.textContent = 'Congratulations, you won!';
                    disableButtons();
                }
            } else {
                ...
                if (guessesRemaining === 0) {
                    gameResultss.textContent = `Sorry, you lost. The word was "${word}".`;
                    disableButtons();
                } else {
                    gameResultss.textContent = 'Incorrect guess.';
                }
            }
